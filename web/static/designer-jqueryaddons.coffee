...
$.fn.clippy = (id, options) ->
    ...
    unless bgcolor
        ...
        if m = bgcolor.match(/^rgb\(\s*(\d+),\s*(\d+)\s*,\s*(\d+)\s*\)$/i)
            r = parseInt(m[1], 10)
            g = parseInt(m[2], 10)
            b = parseInt(m[3], 10)
            bgcolor = '#'+r.toString(16)+g.toString(16)+b.toString(16)

    $(this).append($('<object classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" width="110" height="14" id="clippy"> <param name="movie" value="' + escape(url) + '"/> <param name="allowScriptAccess" value="always" /> <param name="quality" value="high" /> <param name="wmode" value="transparent"/> <param name="scale" value="noscale" /> <param NAME="FlashVars" value="id='+escape(id)+'"> <embed src="' + escape(url) + '" width="110" height="14" name="clippy" quality="high" allowScriptAccess="always" type="application/x-shockwave-flash" pluginspage="http://www.macromedia.com/go/getflashplayer" FlashVars="id='+escape(id)+'" wmode="transparent" /> </object>'))
